...
description: Use when turning a design image, screenshot, Figma export, or mockup into a static website that must visually match the reference, built with Tailwind CSS v4 and zero behavioral JavaScript, and delivered as a full index.html plus per-section HTML and per-section copy-paste prompts.
---
...
1. Tulis `sections/<nn>-<nama>/index.html` — file **standalone lengkap**
   (punya `<head>`, CDN, dan blok `@theme` yang sama), berisi satu section itu saja.
...
3. Tulis `prompts/<nn>-<nama>.md` — prompt siap copy-paste untuk regenerate section
   ...
   COPY, ditutup checklist verifikasi. Struktur 7 blok wajib: aturan dan contoh
   lengkap di `references/prompt-template.md`.

### Step 4 — ASSEMBLE

Gabungkan semua `<section>` ke satu `index.html`:
- Satu `<head>`, satu blok `@theme`.
- Urutan section sesuai nomor.
- Cek jarak antar section konsisten setelah digabung (sering berubah vs versi standalone).
...
```
output/<nama-project>/
├── index.html                    # halaman penuh, siap dibuka
├── NOTES.md                      # token hasil Step 1 + daftar asumsi
├── sections/
│   ├── 01-hero/index.html        # navbar + hero MENYATU di satu file
│   ├── 02-features/index.html
│   └── 03-pricing/index.html     # tabs harga menyatu di sini
└── prompts/
    ├── 01-hero.md                # prompt copy-paste untuk regenerate section ini
    ├── 02-features.md
    └── 03-pricing.md
```
...
| Section standalone tidak punya `@theme` | Tiap file section harus berdiri sendiri. Duplikasi `<head>`-nya. |
...
- `references/prompt-template.md` — cara menulis prompt yang tahan model murah (struktur 7 blok)
